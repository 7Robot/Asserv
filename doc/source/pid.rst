...
Les PID ne sont pas utilisé directement par l’utilisateur, mais par le module
`asserv`. Les prototypes des fonctions sont donc référencés dans
`libasserv_priv.h`.

On peut y trouver également deux structures :

* `PidCoefs`: les coefficiants du PID (kp, ki, kd)
* `Pid`: la structure contenant toutes les données du pid (consigne, erreur,
  terme intégrale, terme dérivé, et bien évidement une structure `PidCoefs`)
...
Outre les setter pour les coefficiants et pour la consigne, le module `pid`
définie 3 fonctions :

* `pid_reset`: Vide le terme intégral.
* `pid_process`: Calcule la prochaine consigne. Pour cela, prend en argument la
  position actuel ainsi que la période (variable `period`, temps entre deux
...
* `pid_done`: Renvoit un boolean indiquant si la consigne a été atteinte. Pour
